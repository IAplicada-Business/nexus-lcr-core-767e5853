...
from gestta.sessao_api import (  # noqa: E402
    SESSION_FILE,
    _gestta_jwt,
    jwt_gestta_quase_expirado,
    ping_gestta_api,
)
...
def garantir_sessao_gestta_api() -> bool:
    ...
    if ping_gestta_api():
        if jwt_gestta_quase_expirado():
            log("  JWT Gestta expira em <2h — tentando relogin preventivo...")
            if relogin_gestta():
                log("  ✓ sessão renovada preventivamente")
            else:
                log("  ⚠️ relogin preventivo falhou — seguindo com sessão ainda válida")
        else:
